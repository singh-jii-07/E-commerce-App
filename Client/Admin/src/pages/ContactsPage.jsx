...
const ContactsPage = () => {
  ...
  const fetchContactsAndSubAdmins = async () => {
    setLoading(true);
    try {
      const [contactRes, userRes] = await Promise.all([
        contactService.getContacts(),
        userService.getAllUsers().catch(() => ({ success: false, users: [] })),
      ]);
      ...
      if (userRes.success && Array.isArray(userRes.users)) {
        const filteredSubAdmins = userRes.users.filter((u) => u.role === "subAdmin");
        setSubAdmins(filteredSubAdmins);
      }
    } catch (err) {
      console.error("Fetch contact resources error:", err);
  ...
  const handleAssign = async (contactId, subAdminId) => {
    try {
      const res = await contactService.assignSupport(contactId, subAdminId || null);
      if (res.success) {
        setContacts((prev) =>
          prev.map((c) => (c._id === contactId ? { ...c, assignedTo: subAdminId || null } : c))
        );
        if (selectedMessage && selectedMessage._id === contactId) {
          setSelectedMessage((prev) => ({ ...prev, assignedTo: subAdminId || null }));
        }
        showFeedback("success", "Ticket assigned successfully.");
      }
    } catch (err) {
      console.error("Assign support error:", err);
      showFeedback("error", err.response?.data?.message || "Failed to assign support.");
    }
  };
        ...
        <div className="fixed inset-0 z-50 bg-black/55 flex items-center justify-center p-4">
          <div className="bg-white border border-gray-200 rounded-xl max-w-lg w-full p-6 shadow-xl relative animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="space-y-4">
              ...
              <div>
                <p className="text-[10px] text-gray-400 uppercase tracking-wider font-semibold">Assigned Support Sub-Admin</p>
                <div className="mt-1">
                  <select
                    value={selectedMessage.assignedTo || ""}
                    onChange={(e) => handleAssign(selectedMessage._id, e.target.value)}
                    className="bg-white border border-gray-200 rounded px-2.5 py-1 text-xs font-semibold text-gray-700 focus:outline-none focus:border-indigo-500 w-full"
                  >
                    <option value="">Unassigned</option>
                    {subAdmins.map((sa) => (
                      <option key={sa._id || sa.authUserId} value={sa.authUserId || sa._id}>
